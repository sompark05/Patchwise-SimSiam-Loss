...
class Launcher(TmuxLauncher):
    def common_options(self):
        return [
            # Command 0
            Options(
                dataroot="datasets/ER/TrainValAB",
                name="test",
                ...
                lambda_simsiam=10.0,  # weight for SimSiam loss
                lambda_simsiam_he=10.0,
          
                # FDL:
                lambda_gp=10.0,
                gp_weights='[0.015625,0.03125,0.0625,0.125,0.25,1.0]',
                lambda_asp=0.0,  # weight for NCE loss: NCE(G(X), X)
                ...
                num_threads=2,  # '# threads for loading data')
                batch_size=1,  # 'input batch size')
                load_size=640,  # 'scale images to this size')
                crop_size=384,  # 'then crop to this size')
