...
targ_dir = '/content/drive/MyDrive/ConvergenceDesignIII/new/AdaptiveSupervisedPatchNCE/results/test/val_latest/images/real_B'
pred_dir = '/content/drive/MyDrive/ConvergenceDesignIII/new/AdaptiveSupervisedPatchNCE/results/test/val_latest/images/fake_B'
...
for i in tqdm(img_list):
    ...
    min_side = min(fake.shape[0], fake.shape[1]) 
    win_size = min(7, min_side if min_side % 2 == 1 else min_side - 1) # odd ≤ min_side
    SSIM = structural_similarity(fake, real, channel_axis=-1, win_size=win_size)
    ssim.append(SSIM)
